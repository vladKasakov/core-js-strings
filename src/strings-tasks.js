...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (ALFABET.includes(str[i].toLowerCase()))
      parsedStr += str[i].toLowerCase();
  }

  for (let i = 0; i < parsedStr.length / 2; i += 1) {
    if (parsedStr[i] !== parsedStr[parsedStr.length - i - 1]) return false;
  }
  return true;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const char = str[i];
    encodedStr += lookupTable[char] || char;
  }
